<template>
	<el-dialog
		...
		<div v-loading="loading" class="forecast-table">
			 <!-- HEADER --> 
		...
		<el-button type="primary" size="medium" :disabled="!schedule_complete" @click="onApprove">APPROVE</el-button>
	</el-dialog>
</template>
	...
	export default {
		...
		methods: {
			onOpen() {
				...
				axios.get('/api/v1/vessel-schedules/forecast/' + this.forecast_ids.service_round_trip_id + '/' + this.forecast_ids.service_proforma_id).then(response => {
					...
					this.legs = packet.vessel_forecast_legs.map(port => {
						...
						if(port.previous_vessel_leg) {
							...
							if(port.previous_vessel_leg.arrival.source.type === 'DURATION') previousLeg.arrival.duration = parseFloat(port.previous_vessel_leg.arrival.source.value)
							else previousLeg.arrival.date = moment(port.previous_vessel_leg.arrival.source.value.date).format('YYYY-MM-DD HH:mm:ss')
							if(port.previous_vessel_leg.departure.source.type === 'DURATION') previousLeg.departure.duration = parseFloat(port.previous_vessel_leg.departure.source.value)
							else previousLeg.departure.date = moment(port.previous_vessel_leg.departure.source.value.date).format('YYYY-MM-DD HH:mm:ss')
			...
			validateSchedule() {
				...
				this.legs.map((port,index) => {
					...
					if(index !== this.legs.length-1) {
						...
						if(!port.arrival.result.duration) valid++;
						if(!port.arrival.result.date) valid++;
					}
					if(valid) incomplete++
				})
			...
			onApprove() {
				...
				data.service_round_trip_id = this.forecast_ids.service_round_trip_id
				data.service_proforma_id = this.forecast_ids.service_proforma_id
				console.log(data)
				axios.post('/api/v1/vessel-schedules/forecast/' + this.forecast_ids.service_round_trip_id + '/' + this.forecast_ids.service_proforma_id, data).then(response => {
