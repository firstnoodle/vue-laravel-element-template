<template>
	<div class="filter">
		...
		<div 
			...
			@click="$store.dispatch(`${store_module_name}/deleteFilter`, id)"
...
export default {
	name: 'DataFilter',	
	props: ['id', 'operator', 'column', 'store_module_name', 'value'],
	data() {
		return {
			columnLabel: null,
	...
	computed: {
		...
		columns() {
			return this.store_module_name ? this.$store.getters[`${this.store_module_name}/columns`] : null
		},
		computedControlsVisible() {
			let column = this.column ? this.$store.getters[`${this.store_module_name}/columnByLabel`](this.column) : null
		...
		computedOperator: {
			...
			set(newValue) { this.$store.dispatch(`${this.store_module_name}/setFilterAttribute`, {id: this.id, attribute: 'operator', value: newValue}) }
		},
		computedColumnType() { return this.column ? this.column.filterType : null },
		computedValue: {
			get() { return this.value },
			set(newValue) { this.$store.dispatch(`${this.store_module_name}/setFilterAttribute`, {id: this.id, attribute: 'value', value: newValue}) }
		}
	},
	watch: {
		deleteActive(newVal, oldVal) { if(!newVal && oldVal) this.$store.dispatch(`${this.store_module_name}/deleteFilter`, this.id) },
	...
	beforeDestroy() {
		console.log(this.$store.state[this.store_module_name].filters)
	},
	methods: {
		onColumnChange() {
			for(const column of this.columns) {
				if(column.label === this.columnLabel) {
					this.$store.dispatch(`${this.store_module_name}/setFilterAttribute`, {id: this.id, attribute: 'value', value: null}) 
					this.$store.dispatch(`${this.store_module_name}/setFilterAttribute`, {id: this.id, attribute: 'column', value: column}) 
				}
			}
		},
		onDecrement(state) {
